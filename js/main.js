(() => {
  ...
  const sceneInfo = [
    ...
    },
    {
      // 1
      type: "normal",
      // heightNum: 5, // 어차피 원래 콘텐츠 높이로 정할거라 주석 처리함.
      scrollHeight: 0,
      objs: {
    ...
    switch (currentScene) {
      ...
      case 3:
        ...
        if (!values.rectStartY) {
          //values.rectStartY = objs.canvas.getBoundingClientRect().top; 스크롤 속도에 따라 값이 달라짐
          // offsetTop은 스크롤 속도에 상관없이 값이 같음
          values.rectStartY =
        ...
        if (scrollRatio < values.rect1X[2].end) {
          ...
          if (scrollRatio > values.blendHeight[2].end) {
            ...
            objs.canvas.style.marginTop = 0; // sticky가 붙으면 marginTop 때문에 안보여서 다시 0으로 만들어줘야 함.
          }
  ...
  // "load" 이미지 포함 모든 요소가 로드되어야 실행
  // "DOMContentLoaded" 이미지나 이런건 로드 안되도 DOM만 로드되면 실행
